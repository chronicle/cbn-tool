...
def call_create_parser(args):
    ...
    if args.skip_validation_on_no_logs == "True":
        data['skip_validation_on_no_logs'] = True
...
def call_archive_parser(args, config_id):
    ...
    parser = make_request(
        args, get_archive_parser_url(args, config_id), 'POST')
    del parser['config']
    print(json.dumps(parser, indent=2))
...
def arg_parser():
    ...
    parser_create_command.add_argument(
        '-s', '--skip_validation_on_no_logs',
        type=str,
        default="False",
        help='Skip validation on no logs (True/False)')
    ...
    dgroup = parser_download_command.add_mutually_exclusive_group(
        required=True)
